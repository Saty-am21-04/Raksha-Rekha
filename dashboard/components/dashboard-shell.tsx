...
export function DashboardShell() {
  ...
  return (
    ...
    <div suppressHydrationWarning className="flex h-dvh flex-col">
      ...
      {/* Non-fatal: the loaded dashboard stays visible underneath. */}
      {view.actionError && (
        <div
          role="alert"
          className="flex shrink-0 items-start justify-between gap-3 border-b border-red-500/40 bg-red-500/10 px-4 py-2"
        >
          <p className="min-w-0 text-xs text-red-300">
            <span className="font-medium">Recompute failed.</span>{" "}
            <span className="break-words text-red-300/80">
              {view.actionError}
            </span>
          </p>
          <button
            type="button"
            onClick={view.dismissActionError}
            aria-label="Dismiss error"
            className="shrink-0 rounded px-2 py-0.5 text-[11px] text-red-300/80 transition-colors hover:bg-red-500/15 hover:text-red-200"
          >
            Dismiss
          </button>
        </div>
